(function ($, Yee) {
    ...
    $.fn.val = function (value) {
        if (value === void 0) {
            if (this.length > 0 && this.getModuleInstance('tinymce')) {
                var id = this.attr('id') || '';
                if (id == '') {
                    return '';
                }
    ...
    Yee.extend('textarea', 'tinymce', function (elem, options) {
        if (!elem.style.width) {
        ...
        $(elem).addClass('tinymce');
        var id = $(elem).attr('id') || '';
        if (id == '') {
            id = 'tinymce_' + randomString(20);
            $(elem).attr('id', id);
        }
        tinymce.init({
            selector: 'textarea#' + id,
            content_style: ".mce-content-body p{margin-top: 10px;margin-bottom:10px;}",
            //  height: 500,
            images_upload_url: options.imagesUploadUrl || '/service/tiny_upfile',
            ...
            init_instance_callback: function (editor) {
                editor.on('GetContent', function (e) {
                    var html = e.content;
                    var reg1 = new RegExp('<\\/?(\\!doctype|html|head|title|body)(\\s[^>]*)?>', 'gi');
                    html = $.trim(html.replace(reg1, ''));
                    var reg2 = new RegExp('<\\/?(p|span|div|br)(\\s[^>]*)?>', 'gi');
                    var text = $.trim(html.replace(reg2, ''));
                    if (text == '') {
                        e.content = '';
                    } else {
                        e.content = $.trim(html);
